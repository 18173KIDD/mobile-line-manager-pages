...
class MobileLineManager {
    ...
    initializeApp() {
        ...
        // スプレッドシートエクスポート機能の設定
        const exportBtn = document.getElementById('exportToSpreadsheet');
        if (exportBtn) {
    ...
    exportToSpreadsheet() {
        // ヘッダー行の準備
        const headers = ['電話番号', '契約日', 'キャリア', '維持期間(月)', '解約可能日', '名義', '利用者', '弾', '備考'];
        ...
        const rows = this.lines.map(line => {
            ...
            if (line.contractDate && line.contractPeriod) {
                const date = new Date(line.contractDate);
                date.setMonth(date.getMonth() + parseInt(line.contractPeriod));
                cancellationDate = date.toLocaleDateString('ja-JP');
            }
            
            return [
                line.phoneNumber,
                line.contractDate ? new Date(line.contractDate).toLocaleDateString('ja-JP') : '',
                line.carrier || '',
                line.contractPeriod || '',
                cancellationDate,
                line.owner || '',
        ...
        link.setAttribute('download', `携帯回線データ_${new Date().toISOString().slice(0, 10)}.csv`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        
        // スプレッドシートへのインポート手順ガイドを表示
        this.showSpreadsheetGuide();
    }
    
    // スプレッドシートへのインポート手順ガイド
    showSpreadsheetGuide() {
        ...
        modal.innerHTML = `
            <div class="option-modal" style="width: 90%; max-width: 500px; max-height: 80vh; overflow-y: auto; position: relative;">
                <h3>スプレッドシートへのインポート手順</h3>
                <ol style="text-align: left; margin: 15px 0; padding-left: 25px;">
                    <li>ダウンロードしたCSVファイルを保存してください</li>
                    <li><a href="https://docs.google.com/spreadsheets" target="_blank">Google スプレッドシート</a>を開く</li>
                    <li>新しいスプレッドシートを作成する</li>
                    <li>メニューから「ファイル」→「インポート」を選択</li>
                    <li>「アップロード」タブからCSVファイルをアップロード</li>
                    <li>インポート設定：
                        <ul>
                            <li>「データを置き換える」を選択</li>
                            <li>「カンマ」区切りを確認</li>
                            <li>「最初の行をヘッダーとして使用する」にチェック</li>
                        </ul>
                    </li>
                    <li>「インポート」をクリック</li>
                </ol>
                <p>これでスプレッドシートでのデータ管理に移行できます！</p>
                <p>さらに次のような活用が可能です：</p>
                <ul style="text-align: left; margin: 15px 0; padding-left: 25px;">
                    <li>解約可能日が近いものを色付け（条件付き書式設定）</li>
                    <li>キャリア別の契約数グラフ作成</li>
                    <li>名義人ごとの契約回線数の集計</li>
                </ul>
                <div style="text-align: center; margin-top: 20px; margin-bottom: 10px;">
                    <button id="closeGuideBtn" class="btn-primary" style="padding: 10px 30px;">閉じる</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
